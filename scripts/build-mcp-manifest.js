...
// pinned `revision` is a real, existing commit (an ancestor of the working
// tree, since a file cannot embed its own commit SHA) — the contract layer
// treats it as "the approved revision", not necessarily HEAD.
//
// Usage:
//   node scripts/build-mcp-manifest.js          # write mcp.manifest.json
//   node scripts/build-mcp-manifest.js --check  # fail if the file is stale
//   node scripts/build-mcp-manifest.js --revision <sha>
...
// Only the digest/manifest are expected to be regenerated on every change.
...
if (check) {
  ...
  if (current !== next) {
    console.error('mcp.manifest.json is stale — run: node scripts/build-mcp-manifest.js');
    process.exit(1);
  }
