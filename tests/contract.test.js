...
//   * action-name parity between the manifest and the real server's tools/list
//   * manifest identity is consistent and namespaced (no core-server collision)
...
test('manifest actions are namespaced and policy-valid', () => {
  ...
  for (const action of source.approvedManifest.actions) {
    assert.match(action.name, /^engineering_[a-z0-9_]*$/, `${action.name} must be namespaced engineering_*`);
    ...
    if (action.effect === 'destructive') assert.equal(action.requiresApproval, true);
  }
  for (const reserved of ['playwright', 'trained-skills', 'hh-skills']) {
